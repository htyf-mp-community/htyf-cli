...
import { scalePx2dp, scaleVu2dp } from '@htyf-mp/taro-runtime-rn'
...
// @ts-ignore
  ...
  init = (config: Config) => {
    this.config = {
      ...
      sass: config.sass || {},
      alias: config.alias ?? {},
      htyf: recursiveMerge({}, DEFAULT_RN_CONFIG, config.htyf)
    }

// @ts-ignore
    Reflect.ownKeys(this.config.htyf || {}).forEach((key: string) => {
      if (
        [
          ProcessTypes.SASS,
          ProcessTypes.LESS,
  ...
  async transform (src: string, filename: string, options: TransformOptions) {
    ...
    const styleObject = transformCSS(
      result.css,
      {
        ...
        scalable: this.config.htyf?.postcss?.scalable
      }
    )
