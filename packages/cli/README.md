...
构建完成后，会在压缩包旁生成 `dist/share-qrcode.png`，并在终端显示分享链接和二维码。扫码进入应用分享页，二维码包含本次构建的应用信息和版本。原有 `dist/dist/qrcode.png` 仍为资源下载二维码。

使用分享二维码前，请将资源包和 `app.json` 分别上传到配置中的 `zipUrl` 和 `appUrlConfig` 地址，供红糖云服 App 拉取与更新。
