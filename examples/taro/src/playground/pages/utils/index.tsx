...
export const isRN = (getEnv() === ENV_TYPE.RN || String(process.env.TARO_ENV) === 'htyf')
...
export function loadWeb({ url, title, right, open, certified = true }: pageType): void {
  if(isWeb) {
    ...
    Taro.navigateTo({
      url: `/playground/pages/webview/index?certified=${certified}&title=${title}&link=${encodeURIComponent(url)}${ right ? `&right=${right}` :''}${ open ? `&open=${encodeURIComponent(open)}` :''}`
    })
  }
}
...
export function loadMini({ url }) {
  Taro.navigateTo({
    url: '/playground/pages/about/mini?src=' + encodeURIComponent(url),
  })
}

export function loadRn({ android, ios }) {
  Taro.navigateTo({
    url: '/playground/pages/about/rn?android=' + encodeURIComponent(android) + '&ios=' + encodeURIComponent(ios),
  })
}
