    ...
    Taro.request({
      ...
      success: res => {
        ...
        function cloneNodeInfo(node) {
          ...
          const newNode = { name: node.name, id: node.id, value: (node.value || []).slice() };
          return newNode;
        }
        function getLevelOption(mode): TreemapSeriesOption['levels'] {
          return [
            {
              color:
                mode === 2
        ...
        function createSeriesCommon(mode): TreemapSeriesOption {
          return {
            type: 'treemap',
            tooltip: {
